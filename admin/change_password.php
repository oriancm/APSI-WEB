...
$success_message = "";
$error_message = "";
$adminLogin = function_exists('envValue') ? envValue('ADMIN_LOGIN', 'admin') : 'admin';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        if ($admin && password_verify($current_password, $admin['pass'])) {
            ...
            $update_sql = "UPDATE admin SET pass = :password WHERE login = :login";
            $update_stmt = $db->prepare($update_sql);
            $update_stmt->bindValue('password', $new_password_hash);
            $update_stmt->bindValue('login', $adminLogin);
