...
if(isset($_POST['commune']) && !empty($_POST['commune'])) {
    $ville = $_POST['commune'];
    $sql = "SELECT * FROM reference WHERE commune LIKE :commune";
    $stmt = $db->prepare($sql);
    $stmt->execute(array(':commune' => '%' . $ville . '%'));
    $refTab = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
<body>
    ...
    <?php include "./navReferenceFiltred.php"; ?>
